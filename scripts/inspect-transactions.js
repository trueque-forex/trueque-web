...
async function main() {
    ...
    `);
        ...
        // Check recent transactions to see status values
        const { rows: txs } = await client.query("SELECT status, count(*) FROM transactions GROUP BY status");
        console.log('\nTRANSACTION STATUS COUNTS:', txs);
