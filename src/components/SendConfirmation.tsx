...
type Props = {
  ...
  onCancel: () => void;
  onProceed: (deliverySpeed: 'instant' | 'same-day' | '48-hours', finalReceive: number) => void;
};
...
export default function SendConfirmation({
  ...
  const finalReceive = useMemo(() => {
    return rate !== 0 ? sendAmount * rate - totalFees : 0;
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gray-50 p-4 rounded shadow-sm space-y-2">
        ...
        <label className="text-sm text-gray-600 block mt-4">
          ...
          <select
            value={deliverySpeed}
            onChange={(e) => setDeliverySpeed(e.target.value as 'instant' | 'same-day' | '48-hours')}
