...
- [ ] **Payment Method Deselection**: In `PaymentMethodForm.tsx`, implement toggle logic so that clicking an already-active payment method card clears the selection (e.g., `onMethodSelect(selectedMethodId === method.id ? '' : method.id)`).
